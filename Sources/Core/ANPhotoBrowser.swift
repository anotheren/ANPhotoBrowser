...
open class ANPhotoBrowser: UIViewController {
    ...
    open weak var photoBrowserDelegate: ANPhotoBrowserDelegate?

    /// 图片加载器
    open var photoLoader: ANPhotoLoader
    ...
    /// 转场动画类型
    open var animationType: ANAnimationType = .scale
    ...
    /// 插件组
    open var plugins: [ANPhotoBrowserPlugin] = []

    /// Cell 插件组
    /// 默认值[ProgressViewPlugin(), RawImageButtonPlugin()]
    open lazy var cellPlugins: [ANPhotoBrowserCellPlugin] = {
        return [ANProgressViewPlugin(), ANRawImageButtonPlugin()]
    }()
    ...
    /// 转场协调器
    private weak var fadePresentationController: ANPhotoBrowserPresentationController?

    /// 缩放型转场协调器
    private weak var scalePresentationController: ANScalePresentationController?
    ...
    public init(animationType: ANAnimationType = .scale,
                delegate: ANPhotoBrowserDelegate? = nil,
                photoLoader: ANPhotoLoader? = nil,
    ...
    /// - parameter presentingVC: 由谁 present 出图片浏览器
    open func show(from viewController: UIViewController? = ANTopMostViewControllerHelper.topMost,
    ...
    /// - returns:  所创建的图片浏览器
    @discardableResult
    open class func show(animationType: ANAnimationType = .scale,
                         delegate: ANPhotoBrowserDelegate,
                         photoLoader: ANPhotoLoader? = nil,
                         plugins: [ANPhotoBrowserPlugin] = [ANPageControlPlugin()],
                         originPageIndex: Int,
                         fromViewController: UIViewController? = ANTopMostViewControllerHelper.topMost
        ) -> ANPhotoBrowser {
        let vc = ANPhotoBrowser(animationType: animationType,
                              delegate: delegate,
    ...
    open class func show(localImages: [UIImage],
                         animationType: ANAnimationType = .fade,
                         delegate: ANPhotoBrowserDelegate? = nil,
                         photoLoader: ANPhotoLoader? = nil,
                         plugins: [ANPhotoBrowserPlugin] = [ANPageControlPlugin()],
                         originPageIndex: Int,
...
extension ANPhotoBrowser: UICollectionViewDataSource {
    ...
    public func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(ANPhotoBrowserCell.self, for: indexPath)
    ...
    public func collectionView(_ collectionView: UICollectionView, willDisplay cell: UICollectionViewCell, forItemAt indexPath: IndexPath) {
        if let pbCell = cell as? ANPhotoBrowserCell {
            cellPlugins.forEach {
...
extension ANPhotoBrowser: UICollectionViewDelegate {
    /// 减速完成后，计算当前页
...
extension ANPhotoBrowser: UIViewControllerTransitioningDelegate {
    /// 提供进场动画
    ...
    public func presentationController(forPresented presented: UIViewController, presenting: UIViewController?, source: UIViewController) -> UIPresentationController? {
        switch animationType {
        case .scale:
            let controller = ANScalePresentationController(presentedViewController: presented, presenting: presenting)
        ...
        case .scaleNoHiding:
            ...
            return controller
        case .fade:
            let controller = ANFadePresentationController(presentedViewController: presented, presenting: presented)
            fadePresentationController = controller
            return controller
        }
    }
    ...
    private func makeScalePresentationAnimator(indexPath: IndexPath) -> UIViewControllerAnimatedTransitioning {
        let cell = collectionView.cellForItem(at: indexPath) as? ANPhotoBrowserCell
        let imageView = UIImageView(image: cell?.imageView.image)
        imageView.contentMode = imageScaleMode
        imageView.clipsToBounds = true
        // 创建animator
        return ANScaleAnimator(startView: relatedView, endView: cell?.imageView, scaleView: imageView)
    }

    /// 创建缩放型退场动画
    private func makeDismissedAnimator() -> UIViewControllerAnimatedTransitioning? {
        guard let cell = collectionView.visibleCells.first as? ANPhotoBrowserCell else {
            return nil
        }
        let imageView = UIImageView(image: cell.imageView.image)
        imageView.contentMode = imageScaleMode
        imageView.clipsToBounds = true
        return ANScaleAnimator(startView: cell.imageView, endView: relatedView, scaleView: imageView)
    }
}
...
extension ANPhotoBrowser: ANPhotoBrowserCellDelegate {

    public func photoBrowserCell(_ cell: ANPhotoBrowserCell, didSingleTap image: UIImage?) {
    ...
    public func photoBrowserCell(_ view: ANPhotoBrowserCell, didPanScale scale: CGFloat) {
        // 实测用scale的平方，效果比线性好些
    ...
    public func photoBrowserCell(_ cell: ANPhotoBrowserCell, didLongPressWith image: UIImage, gesture: UILongPressGestureRecognizer) {
